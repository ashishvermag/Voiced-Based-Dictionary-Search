...
void read_lambda_values(int digit)
{
	FILE *fpt;
	char filename[40];
	sprintf(filename,"lambda_values/%d_pi.txt",digit);
	fpt=fopen(filename,"r");
	for(int i=1;i<=N;i++)
		fscanf(fpt,"%lf",&pi[i]);
	fclose(fpt);
	sprintf(filename,"lambda_values/%d_A.txt",digit);
	fpt=fopen(filename,"r");
	for(int i=1;i<=N;i++)
	{
		for(int j=1;j<=N;j++)
	...
	fclose(fpt);
	sprintf(filename,"lambda_values/%d_B.txt",digit);
	fpt=fopen(filename,"r");
	for(int i=1;i<=N;i++)
	{
		for(int j=1;j<=M;j++)
...
void converge_model(int train_count,int digit)
{
	...
	do
	{
		...
		//printf("new pstar= %Lf",new_Pstar);
		soln_problem3();
	}while(countt!=1000 && old_Pstar<new_Pstar);
}
...
void training()
{
	...
	for(int i=1;i<=10;i++)
	{
		...
		printf("training_sample/%d\n",i);
		for(int j=1;j<=sample_count;j++)
		{
			sprintf(filename,"training_sample/%d_%d.txt",i,j);
